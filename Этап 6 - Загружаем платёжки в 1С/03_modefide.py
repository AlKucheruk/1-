...
MAPPING_FILE = SCRIPT_DIR / "Маппинг Операция и Статья ДДС.xlsx"
...
def apply_mapping(df: pd.DataFrame, mapping_dict: dict) -> pd.DataFrame:
    df = df.copy()
...
def save_results_by_year(input_path: Path, df_result: pd.DataFrame):
    ...
    # ================================================
    # ДОБАВЛЕНО: сохранение общего файла со всеми данными
    # ================================================
    all_output_name = f"{input_path.stem}_ALL_prepared.xlsx"
    all_output_path = input_path.with_name(all_output_name)
    
    save_single_result(all_output_path, df_result)   # используем существующую функцию
    saved_files.append(all_output_path)
    # ================================================
...
def main():
    try:
        ...
        df_result = apply_mapping(df_source, mapping_dict)
